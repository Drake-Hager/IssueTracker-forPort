...
export default function UserEditor({showError, showSuccess}){
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    let response = await axios.patch(`${import.meta.env.VITE_API_URL}/api/users/${userId}`,{email:userEmail, password: userPassword, givenName:userFirstName, lastName:userFamilyName} ,{withCredentials:true})
  ...
  const fetchUser = async () =>{
    let response2 = await axios.get(`${import.meta.env.VITE_API_URL}/api/users/${userId}`, {withCredentials:true})
